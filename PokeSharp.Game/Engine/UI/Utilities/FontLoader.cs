...
namespace PokeSharp.Game.Engine.UI.Debug.Utilities;
...
public static class FontLoader
{
    ...
    /// </summary>
    private const string BundledFontResourceName =
        "PokeSharp.Game.Engine.UI.Debug.Assets.Fonts.0xProtoNerdFontMono-Regular.ttf";
    
    /// <summary>
    ///     Relative path to the font file in the output directory.
    /// </summary>
    private const string BundledFontFilePath =
        "Assets/Fonts/0xProtoNerdFontMono-Regular.ttf";
    ...
    public static FontSystem? LoadFont()
    {
        ...
        FontSystem? fontSystem = LoadBundledFont();
        if (fontSystem != null)
        {
            return fontSystem;
        }
    ...
    /// <summary>
    ///     Attempts to load the bundled Iosevka Nerd Font from embedded resources or file system.
    /// </summary>
    /// <returns>A FontSystem with the bundled font, or null if not found.</returns>
    public static FontSystem? LoadBundledFont()
    {
        // First, try loading from file system (works after consolidation)
        if (File.Exists(BundledFontFilePath))
        {
            try
            {
                byte[] fontData = File.ReadAllBytes(BundledFontFilePath);
                var fontSystem = new FontSystem();
                fontSystem.AddFont(fontData);
                Console.WriteLine($"[FontLoader] Loaded bundled font from file: {BundledFontFilePath}");
                return fontSystem;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[FontLoader] Failed to load font from file: {ex.Message}");
                // Fall through to embedded resource loading
            }
        }
        else
        {
            Console.WriteLine($"[FontLoader] Font file not found at: {BundledFontFilePath}");
        }

        // Try loading from embedded resources (legacy path)
        var assembly = Assembly.GetExecutingAssembly();

        // Try the primary resource name
        Stream? stream = assembly.GetManifestResourceStream(BundledFontResourceName);

        // If not found, search for any matching resource
        if (stream == null)
        {
            string[] resourceNames = assembly.GetManifestResourceNames();
            foreach (string pattern in AlternativeResourcePatterns)
            {
                string? matchingResource = resourceNames.FirstOrDefault(r =>
