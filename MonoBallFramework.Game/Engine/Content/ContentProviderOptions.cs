...
public class ContentProviderOptions
{
    ...
    public string GetContentFolder(string contentType)
    {
        ...
        if (BaseContentFolders.TryGetValue(contentType, out string? folder))
        {
            return folder;
        }

        // Check custom content types
        if (CustomContentTypes.TryGetValue(contentType, out string? customFolder))
        {
            return customFolder;
        }
    ...
    public void Validate()
    {
        ...
        {
            throw new ArgumentException("BaseContentFolders must contain at least one mapping.",
                nameof(BaseContentFolders));
        }

        foreach (KeyValuePair<string, string> kvp in BaseContentFolders)
        {
            if (string.IsNullOrWhiteSpace(kvp.Key))
            {
                throw new ArgumentException("BaseContentFolders cannot contain null or empty keys.",
                    nameof(BaseContentFolders));
            }
            ...
            {
                throw new ArgumentException($"BaseContentFolders value for '{kvp.Key}' cannot be null or empty.",
                    nameof(BaseContentFolders));
            }
        }

        // Validate CustomContentTypes entries
        foreach (KeyValuePair<string, string> kvp in CustomContentTypes)
        {
            if (string.IsNullOrWhiteSpace(kvp.Key))
            {
                throw new ArgumentException("CustomContentTypes key cannot be null or empty.",
                    nameof(CustomContentTypes));
            }

            if (string.IsNullOrWhiteSpace(kvp.Value))
            {
                throw new ArgumentException($"CustomContentTypes value for '{kvp.Key}' cannot be null or empty.",
                    nameof(CustomContentTypes));
            }
            ...
            if (kvp.Value.Contains(".."))
            {
                throw new ArgumentException(
                    $"CustomContentTypes path for '{kvp.Key}' cannot contain path traversal sequences.",
                    nameof(CustomContentTypes));
            }
        }
    }
