...
public class TimeCommand : IConsoleCommand
{
    ...
    public string Usage => @"time                    Show current time state
  ...
  time slowmo 25        Run at 25% speed";
    ...
    public Task ExecuteAsync(IConsoleContext context, string[] args)
    {
        ...
        if (timeControl == null)
        {
            ...
            context.WriteLine("  • ITimeControl was not found in loaded assemblies", theme.TextSecondary);
            context.WriteLine("  • ITimeControl is not registered in the DI container", theme.TextSecondary);
            context.WriteLine("", theme.TextPrimary);
            context.WriteLine("Check the console logs (enable with 'logging on') for details.", theme.TextSecondary);
        ...
        switch (subCommand)
        {
            ...
            case "speed":
                if (args.Length < 2)
                {
                    context.WriteLine($"Current time scale: {timeControl.TimeScale:F2}x", theme.Info);
                    context.WriteLine("Usage: time scale <value>  (e.g., time scale 0.5)", theme.TextSecondary);
                    break;
                }

                // Use InvariantCulture to ensure "0.5" parses correctly regardless of locale
                if (float.TryParse(args[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var scale))
                {
                    scale = Math.Clamp(scale, 0f, 10f);
                    timeControl.TimeScale = scale;

                    var message = scale switch
                    {
                ...
                else
                {
                    ...
                    context.WriteLine("Usage: time scale <number>  (e.g., 0.5, 1, 2)", theme.TextSecondary);
                }
                break;
            ...
            case "slow":
                if (args.Length < 2)
                {
                    var currentPercent = timeControl.TimeScale * 100;
                    context.WriteLine($"Current speed: {currentPercent:F0}%", theme.Info);
                    context.WriteLine("Usage: time slowmo <percent>  (e.g., time slowmo 25)", theme.TextSecondary);
                    break;
                }

                // Use InvariantCulture for consistent decimal parsing
                if (float.TryParse(args[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var percent))
                {
                    // Convert percentage to scale (25% = 0.25)
                    var slowScale = Math.Clamp(percent / 100f, 0f, 10f);
                    timeControl.TimeScale = slowScale;
                    context.WriteLine($"Speed set to {percent:F0}% (scale = {slowScale:F2})", theme.Success);
                }
                else
                {
                    context.WriteLine($"Invalid percentage: {args[1]}", theme.Error);
                    context.WriteLine("Usage: time slowmo <percent>  (e.g., 25 for 25% speed)", theme.TextSecondary);
                }
                break;
            ...
            default:
                ...
                context.WriteLine("  time step [frames]      Step N frames when paused", theme.TextSecondary);
                context.WriteLine("  time scale <value>      Set time scale (0.5, 1, 2, etc.)", theme.TextSecondary);
                context.WriteLine("  time slowmo <percent>   Set speed percentage (25, 50, etc.)", theme.TextSecondary);
                break;
        }
    ...
    private static void ShowTimeState(IConsoleContext context, Engine.Core.Services.ITimeControl timeControl)
    {
        var theme = context.Theme;
        var scale = timeControl.TimeScale;
        var isPaused = timeControl.IsPaused;
        ...
        // Status
        var status = isPaused ? "PAUSED" : "RUNNING";
        var statusColor = isPaused ? theme.Warning : theme.Success;
        ...
        context.WriteLine("", theme.TextPrimary);
        context.WriteLine("Quick commands: pause | resume | step | scale <n> | slowmo <n>", theme.TextDim);
    }
}
...
public class PauseCommand : IConsoleCommand
{
    ...
    public Task ExecuteAsync(IConsoleContext context, string[] args)
    {
        ...
        if (timeControl == null)
        {
            context.WriteLine("⚠ Time control is not available. Run 'time' for details.", context.Theme.Warning);
            return Task.CompletedTask;
        }

        timeControl.Pause();
        context.WriteLine("Game paused", context.Theme.Warning);
        context.WriteLine("Use 'resume' to continue, 'step' to advance frames", context.Theme.TextSecondary);
        return Task.CompletedTask;
    }
}
...
public class StepCommand : IConsoleCommand
{
    ...
    public string Usage => @"step [frames]    Step forward N frames (default: 1)
...
Examples:
  ...
  step 60        Advance ~1 second at 60fps";
    ...
    public Task ExecuteAsync(IConsoleContext context, string[] args)
    {
        ...
        if (timeControl == null)
        {
            context.WriteLine("⚠ Time control is not available. Run 'time' for details.", theme.Warning);
            return Task.CompletedTask;
        }
