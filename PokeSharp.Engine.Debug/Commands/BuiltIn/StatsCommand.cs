...
public class StatsCommand : IConsoleCommand
{
    ...
    public string Usage =>
        @"stats                  - Show current performance stats
...
stats summary          - Show brief performance summary";
    ...
    public Task ExecuteAsync(IConsoleContext context, string[] args)
    {
        ...
        IStatsOperations? stats = context.Stats;
    ...
    private static void ShowFps(IConsoleContext context, IStatsOperations stats)
    {
        UITheme theme = context.Theme;
        float fps = stats.CurrentFps;
        Color fpsColor =
            fps >= 55 ? theme.Success
    ...
    private static void ShowFrameTime(IConsoleContext context, IStatsOperations stats)
    {
        UITheme theme = context.Theme;
        (float min, float max, float avg) = stats.FrameTimeStats;
        float current = stats.CurrentFrameTimeMs;

        Color frameColor =
            current <= 16.67f ? theme.Success
            : current <= 25f ? theme.Warning
            : theme.Error;
        ...
        context.WriteLine(
            $"  Max:     {max:F2}ms",
            max > 25f ? theme.Warning : theme.TextSecondary
        );
        context.WriteLine("  Budget:  16.67ms (60 FPS)", theme.TextDim);

        // Budget usage
        float budgetPercent = current / 16.67f * 100f;
        Color budgetColor =
    ...
    private static void ShowMemory(IConsoleContext context, IStatsOperations stats)
    {
        UITheme theme = context.Theme;
        double memMB = stats.CurrentMemoryMB;
        Color memColor =
            memMB < 256 ? theme.Success
            : memMB < 512 ? theme.Warning
            : theme.Error;
    ...
    private static void ShowGC(IConsoleContext context, IStatsOperations stats)
    {
        UITheme theme = context.Theme;
        (int gen0, int gen1, int gen2) = stats.GCCollections;

        context.WriteLine("═══ Garbage Collection ═══", theme.Info);
        context.WriteLine($"  Gen0: {gen0} collections", theme.TextPrimary);
        context.WriteLine(
            $"  Gen1: {gen1} collections",
            gen1 > 10 ? theme.Warning : theme.TextSecondary
        );
        context.WriteLine(
            $"  Gen2: {gen2} collections",
            gen2 > 0 ? theme.Error : theme.TextSecondary
        );
    ...
    private static void ShowSummary(IConsoleContext context, IStatsOperations stats)
    {
        UITheme theme = context.Theme;
        float fps = stats.CurrentFps;
        float frameTime = stats.CurrentFrameTimeMs;
        double memMB = stats.CurrentMemoryMB;
        int entities = stats.CurrentEntityCount;
        (int gen0, int gen1, int gen2) = stats.GCCollections;
        ...
        Color fpsColor =
            ...
            : theme.Error;
        Color frameColor =
            frameTime <= 16.67f ? theme.Success
            : frameTime <= 25f ? theme.Warning
            : theme.Error;
        Color memColor =
            memMB < 256 ? theme.Success
            : memMB < 512 ? theme.Warning
        ...
        context.WriteLine(
            ...
            gen2 > 0 ? theme.Warning : theme.TextSecondary
        );

        // Overall health
        bool isHealthy = fps >= 55 && memMB < 512 && gen2 < 5;
        context.WriteLine(
            $"  Status:     {(isHealthy ? "✓ Healthy" : "⚠ Check profiler")}",
            isHealthy ? theme.Success : theme.Warning
        );
    }
}
